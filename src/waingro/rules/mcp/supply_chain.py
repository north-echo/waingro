...
class RugPullIndicator(MCPRule):
    ...
    _deferred_patterns = [
        ...
        re.compile(
            r"new\s+Date\s*\(\s*\).*(?:getMonth|getFullYear|getDate).*(?:tool|handler|exec|eval)"
        ),
        ...
        for hook in self._lifecycle_hooks:
            if hook in scripts:
                ...
                if not isinstance(script_content, str):
                    continue
                if LIFECYCLE_EXEC_OR_FETCH_RE.search(script_content):
                    findings.append(
                        Finding(
                            rule_id=self.rule_id,
                            title=f"Suspicious {hook} lifecycle hook",
                            description=(
                                f"The '{hook}' script contains potentially malicious patterns"
                            ),
                            ...
                            remediation=(
                                ...
                                "They should not download code, use eval, or make "
                                "network requests."
                            ),
